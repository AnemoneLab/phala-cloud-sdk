...
/**
 * 此示例演示如何使用Phala Cloud SDK部署新的Docker容器
 * 
 * 要运行此示例，您需要:
 ...
 * 
 * 运行命令: 
 * ts-node deploy-new-app.ts <app_name>
 */
...
async function main() {
  try {
    ...
    if (process.argv.length < 3) {
      console.error('缺少必要参数: app_name');
      console.error('用法: ts-node deploy-new-app.ts <app_name>');
      process.exit(1);
    }
    ...
    // 使用SDK的部署监控方法
    const finalStatus = await phalaCloud.monitorDeploymentStatus(deployResult.app_id, {
      // 状态变化回调
